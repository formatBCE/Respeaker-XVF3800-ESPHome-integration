...
1. Review the example YAML in `config/respeaker-xvf-satellite-example.yaml` and copy the relevant sections into your ESPHome device configuration (for example `/config/esphome/respeaker-xvf3800.yaml`). The example already uses `id(timer_ringing).is_on()` and `id(va).cancel_timeout(...)` to avoid compilation errors with recent ESPHome versions.
