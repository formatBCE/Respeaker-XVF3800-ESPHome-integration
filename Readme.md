...
## Overview
This repository provides custom ESPHome components, an example configuration, and helper scripts for running a Respeaker XVF3800 USB 4-mic array as a Home Assistant voice satellite. The goal is to make the XVF3800 usable over I2C with ESPHome, including firmware management and some device state exposure. Firmware binaries are not stored in this repo; host them separately and reference them in your ESPHome config.
...
3. Optionally include the firmware block in the config to perform DFU updates from a hosted `.bin` file.
